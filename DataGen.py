...
def ideal_data(min_x=0, max_x=100, num=20,
               coeffs=[25, 0.1, 10, 30]):
    """ideal data generated by equation A*sin(Bx+C)+D
    args:
        min_x: (default=0) min x co-ordinate,
        max_x: (default=100) max x co-ordinate,
        num: (default=20) total number of points
        coeffs: list of len 4 [A, B, C, D]
    """
    # equallly spaced points in x
    x = np.linspace(min_x, max_x, num)
    # y = coeffs[0] * x**2 + coeffs[1] * x + coeffs[2] #[-0.02, 2, 1]
    # y = -0.02 * x**2 + 2 * x + 1

    # obtain y by equation
    y = coeffs[0] * np.sin(coeffs[1]*x + coeffs[2]) + coeffs[3]
    
    '''orientation computation, 
    obtain the vector that joins current point and 
    next then compute the arctan(y2-y1, x2-x1)'''

    # get x diff between point(n+1) and point(n)
    x_diff = np.diff(x)
    # get y diff between point(n+1) and point(n)
    y_diff = np.diff(y)
...
def get_data(min_x=0, max_x=100, num=20,
    ...
    args:
        ...
        num: total points
        coeffs: path equation coeffs for equation A*sin(Bx+C)+D as [A, B, C, D]
        dev: 1D array specifying standard deviation (noise to be added) in [x, y, theta]
        missing_data: 1D array of indices where data is to be numpy.NaN (no/invalid measurement)
    
    returns:
        tuple of size 3 the following
        numpy.array(x, y, t), #ideal robot positions
        numpy.array(x_in, y_in, t_in), #noisy data
        numpy.array(x_acc, y_acc, t_acc) #noisy acceleration data at each position (imu)

    """
    # true values
    # 2 more points requested as second order differentials will be computed to obtain acceleration
    x, y, t = ideal_data(min_x=min_x, max_x=max_x, num=num+2,
                         coeffs=coeffs)
                         
    # noisy data
    # seed is fixed so that similar results are obtained 
    # normal/gaussian noise is added with given standard deviation
    ...
    the velocity is computed by taking difference of the point in a dimension and dividing by time factor
    here time stamp is considered as 1 unit
    velocity in y at point n (v(n))= (y(n+1)-y(n))/t where t = 1
    acceleration is the change in velocity from point n to n+1 hence can be obtained as
    acceleration(n) = v(n+1)-v(n) in x and y dimension respectively
    the acceleration noise is added to simulate noisy inputs from imu's
    '''
    ...
    # missing data
    # replace missing data points by np.NaN to simulate missing data
    for idx in missing_data:
        x_ip[idx] = np.NaN
        y_ip[idx] = np.NaN
        t_ip[idx] = np.NaN

    # return true_data, input_noisy_data, imu_data
...
if __name__ == '__main__':
    '''To check if generated data is as desired
    '''
